...
\subsection*{Standing Broad Jump}

\columntwo{
    \item It's a maximal test, so the amount of power measured is true
    \item Little equipment needed
    \item Instructions are simple
}{
    \item The reliability of the test is questionable since over time the subject's technique may improve
    \item The reliability of the test is also affected by the environment, like the material of the track and shoes
    \item Novice subjects may be injured from lack of technique
    \item Maximal test, so motivation may affect the amount of distance traveled
}
